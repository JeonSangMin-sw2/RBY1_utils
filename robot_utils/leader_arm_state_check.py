        ...
        try:
            ...
            for tid in self.active_tool_ids:
                try:
                    ...
                    new_tool_faults.append(tid)
                    self.state.fault_ids_history[tid - 0x80 + self.DOF] += 1
                    logging.warning(f"Exception reading tool {tid}: {e}")
            ...
            try:
                ...
                logging.warning(f"get_motor_states threw exception: {e}")
            ...
            if ms_list and len(ms_list) == len(self.active_joint_ids):
                ...
                if self.goal_pos_flag:
                    try:
                        ...
                        logging.warning(f"Error reading goal position: {e}")
                ...
                for i, mid in enumerate(self.motor_ids):
                    try:
                        ...
                        if mode is None:
                            ...
                            logging.warning(f"[LeaderArm] Daisy-chain break at ID {mid}. Faulted: {broken_ids}")
                            break
                    except Exception as e:
                        broken_ids = self.motor_ids[i:]
                        self.state.joint_fault_ids = broken_ids
