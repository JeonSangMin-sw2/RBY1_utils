...
package_dir=${1:?"usage: $0 DIST_ONEDIR REPRESENTATIVE_FIXTURES"}
fixtures=${2:?"usage: $0 DIST_ONEDIR REPRESENTATIVE_FIXTURES"}

if [[ $(id -u) -eq 0 ]]; then
  echo "refusing privileged package smoke" >&2
  exit 2
fi
[[ -d "$package_dir" ]] || { echo "missing onedir package: $package_dir" >&2; exit 2; }
[[ -e "$fixtures" ]] || { echo "missing fixtures: $fixtures" >&2; exit 2; }
...
launcher="$package_dir/rby1-cs-analyzer-v4"
cli="$package_dir/rby1-cs-analyzer-v4-cli"
[[ -x "$launcher" && -x "$cli" ]] || { echo "package entrypoints are not executable" >&2; exit 2; }
...
echo "PASS: offline unprivileged onedir smoke"
